...
class RegexTest(StageTest):
    m_cases = [
        ...
        # 1 ("", "apple", "true", "An empty regex always returns true!"),
        # 2 ("apple", "", "false", "A non-empty regex and an empty input string always returns false!"),
        ...
        # 3 ("apwle", "apple", "false", "Two different patterns should not return true!"),
        # 4 ("peach", "apple", "false", "Two different patterns should not return true!"),
        # stage 4
        ('^app', 'apple', "true",
        ...
        # ('^a', 'apple', "true",
        # 5 "A regex starting with '^' should match the following pattern only at the beginning of the input string!"),
        ('.$', 'apple', "true",
         "A regex ending with '$' should match the preceding pattern only at the end of the input string!"),
        # ('apple$', 'tasty apple', "true",
        # 6 "A regex ending with '$' should match the preceding pattern only at the end of the input string!"),
        # ('^apple', 'apple pie', "true",
        # 7 "A regex starting with '^' should match the following pattern only at the beginning of the input string!"),
        ('^apple$', 'apple', "true",
        ...
        # 8 "A regex starting with '^' and ending with '$' should match only the enclosed literals!"),
        ('app$', 'apple', "false",
         "A regex ending with '$' should match the preceding pattern only at the end of the input string!"),
        ('^le', 'apple', "false",
         "A regex starting with '^' should match the following pattern only at the beginning of the input string!"),
        ...
        # stage 6
        ("\\.$", "end.", "true",
         "Don't forget that '\\' is an escape symbol in Go itself, so it has to be duplicated!"),
        ("3\\+3", "3+3=6", "true",
         "Don't forget that '\\' is an escape symbol in Go itself, so it has to be duplicated!"),
